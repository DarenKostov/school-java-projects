...
class CommandWords
{
    ...
    private static final String validCommands[][] = {
    		{"help", "h"}, {"features", "f"}, {"tips", "ti"}, {"go", "g", "goto"} , {"take", "t"}, {"use", "u", "utilize"}, {"drop", "d"}, {"inventory", "i", "inv"}, {"quit", "q", "exit", "e", "leave"}, 
    		{"look", "l", "lookaround", "la"}
    };
    private static final String CommandsDescription[] = {
    		"this help", "gives a list of all the features", "gives you tips", "$ moves your character in a direction", "$ takes a single item with you", "$ uses an item you have on you",
    		"$ leaves an item you have on the ground", "$ shows your inventory", "exits the game", "$ describes the room you are in once again"
    };
    ...
    public boolean isCommand(String aString)
    {
        for(int i = 0; i < validCommands.length; i++) {
            for(int j=0; j<validCommands[i].length; j++) {
                if(validCommands[i][j].equals(aString))
                    return true;
            }
        }
